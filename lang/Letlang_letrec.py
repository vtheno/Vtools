...
dt.expression('a') ==  \
    ...
    |  c.letrec_exp('proc_name','bound_var','proc_body','letrec_body') \
...
# letrec proc_name (bound_var) = proc_body in letrec-body
...
def extend_env_rec(pname,bvar,pbody,env):
    # pname : identifier
    # bvar  : identifier
    # pbody  : expression
    # env   : environment
    return ['extend-env-rec',pname,bvar,pbody,env]
...
def apply_env(env,search_var):
    ...
    if car(env) == empty:
        ...
        bvar  = cadr(tmpEnv)
        pbody = caddr(tmpEnv)
        saved_env = cadddr(tmpEnv)
        if search_var == pname : # pname is procedure name
...
def value_of(self,env):
    return value_of(self.letrec_body,extend_env_rec(self.proc_name,self.bound_var,self.proc_body,env))
init = init_env()
empty = empty_env()

def pLetrec(inp):
    sym = symbol(toList("letrec"))
    defn = symbol(toList("="))
    In   = symbol(toList("in"))
    lf = symbol(toList("("))
    rf = symbol(toList(")"))
    bk = bracket(lf)(var)(rf)
    return bind(sym,lambda _:
           bind(var,lambda pname:
           bind(bk,lambda bvar:
            bind(defn,lambda _:
            bind(expr,lambda pbody:
            bind(In,lambda _:
            bind(expr,lambda lbody:
                 mret( letrec_exp(pname,bvar,pbody,lbody) ) )))))))(inp)

print( read(expr)("""
letrec 
    double (x) = if zero? x
                 then 0
                 else -((double -(x 1)) minus(2))
in (double 6)
""" ).value_of(empty) )
